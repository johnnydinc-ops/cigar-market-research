...
import { AffiliateFeedConnector, runPipeline, createObservability, } from "@cigar/ingestion";
const PORT = Number(process.env.PORT) || 3002;
const KILL_SWITCH = process.env.INGESTION_KILL_SWITCH === "true";
async function main() {
    const observability = createObservability();
    const server = await import("http").then((http) => http.createServer(async (req, res) => {
        if (req.url === "/health") {
