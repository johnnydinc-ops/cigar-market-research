...
function packLabelToPackSize(label) {
...
function extractProductsFromListing(html) {
    ...
    productLinks.each((_, el) => {
        ...
        if (seen.has(href))
            return;
        seen.add(href);
        const title = $el.find(".product-name, .name, .title, h2, h3").first().text().trim() ||
            $el.text().trim().split("\n")[0]?.trim() ||
            "";
        ...
        for (const line of lines) {
            if (line === title || line.startsWith("$") || /^\d+$/.test(line))
                continue;
        ...
        if (!shortSpec && $card.length) {
            ...
            if (specText.length > 10 && specText.length < 400)
                shortSpec = specText;
        }
...
function extractProductDetail(html, _productUrl) {
    const $ = load(html);
    ...
    if (!description) {
        ...
        if (match)
            description = match[1].trim().slice(0, 4096);
    }
    ...
    $tables.each((_, table) => {
        ...
        $rows.each((_, tr) => {
            ...
            if (!packLabel || packLabel.length > 80)
                return;
            if (/^(MSRP|OUR PRICE|YOU SAVE|SMOKE RINGS|AVAILABILITY|QUANTITY)$/i.test(packLabel.trim()))
                return;
...
export function createNeptuneFetch(options) {
    ...
    return async (opts) => {
        ...
        for (const productUrl of urlsToDetail) {
            ...
            try {
                ...
                detailCount++;
                if (detailCount % 20 === 0)
                    opts.observability?.onItem?.("neptune", "document", detailCount);
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
...
export const NEPTUNE_TOS_METADATA = {
...
export function createNeptuneConnector(tosMetadata = NEPTUNE_TOS_METADATA, opts) {
