 ...
 */
export async function discoverUrlsFromSitemap(sitemapUrl, options = {}) {
    ...
    try {
        ...
        for (const loc of locs) {
            if (urls.length >= maxUrls)
                break;
            const href = loc.replace(/<\/?loc>/gi, "").trim();
            if (!href || !pathPattern.test(href) || seen.has(href))
                continue;
            seen.add(href);
            urls.push(href);
        }
 ...
 */
export function urlsToListingSeeds(urls, _baseUrl, maxSeeds = 20) {
