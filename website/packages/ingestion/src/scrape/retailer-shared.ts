...
export function parsePriceCents(text: string | undefined): number | null {
  if (!text) return null;
...
export function packLabelToPackSize(label: string): PackSize {
...
export function parseTitle(title: string): { brand: string; line?: string; vitola?: string } {
  const t = title.trim();
...
export interface ProductDetail {
  ...
  reviews: Array<{ author: string; date: string; text: string }>;
  priceRows: PriceRow[];
}

/** Extract product title from a detail page (h1, og:title, .product-name, <title>). */
export function extractProductTitleFromDetail(html: string): string | null {
  const $ = load(html);
  const selectors = [
    "h1.product-name",
    "h1.product-title",
  ...
  for (const sel of selectors) {
    ...
    if (t && t.length >= 2 && t.length < 300 && !/^\s*\$|^add to cart|^buy now$/i.test(t)) return t;
  }
  const ogTitle = $('meta[property="og:title"]').attr("content")?.trim();
  if (ogTitle && ogTitle.length >= 2 && ogTitle.length < 300) return ogTitle;
  const titleTag = $("title").text().trim();
  if (titleTag) {
    const clean = titleTag.replace(/\s*[\|\-–—]\s*.*$/, "").trim();
    if (clean.length >= 2 && clean.length < 300) return clean;
  }
  return null;
}

/** Generic product-detail extraction (description, spec, rating, reviews, pricing). Works across many cigar retailer sites. */
export function extractProductDetailGeneric(html: string): ProductDetail {
  const $ = load(html);
  ...
  if (!description && bodyNorm.length > 200) {
    ...
    if (match) description = match[0]!.trim().slice(0, 4096);
  }
  ...
  $("strong, b, dt").each((_, el) => {
    ...
    if (rest && !/^(MSRP|OUR PRICE|YOU SAVE|QUANTITY)/i.test(rest)) specParts.push(`${label}: ${rest}`);
  });
  ...
  $("table tr").each((_, tr) => {
    ...
    if (texts.length < 2 || !/\$\d/.test(texts.join(" "))) return;
    const packLabel = texts[0] ?? "";
    if (!packLabel || packLabel.length > 80 || /^(MSRP|OUR PRICE|YOU SAVE|QUANTITY)$/i.test(packLabel.trim())) return;
...
export function extractListingProductsGeneric(
  html: string,
  baseUrl: string,
  options?: { productPathPattern?: RegExp; linkSelector?: string }
): ListingProduct[] {
  const $ = load(html);
  ...
  $(selector).each((_, el) => {
    ...
    if (!href || !pattern.test(href) || href.startsWith("#")) return;
