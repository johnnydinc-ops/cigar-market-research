...
export function browserLikeHeaders(origin?: string): Record<string, string> {
  return {
    "User-Agent":
 ...
 */
export function createFetchFetcher(options: FetchFetcherOptions = {}): FetchPageFn {
  const browserLike = options.browserLike !== false;
  const retries = options.retries ?? DEFAULT_RETRIES;

  return async (url: string, opts: ScrapeFetchOptions): Promise<string> => {
...
export async function createPlaywrightFetcher(
...
): Promise<{ fetchPage: FetchPageFn; close: () => Promise<void> }> {
  ...
  const fetchPage: FetchPageFn = async (url: string, opts: ScrapeFetchOptions): Promise<string> => {
    if (opts.rateLimiter && !opts.rateLimiter.allow()) {
      throw new Error("Rate limit exceeded");
    }
