...
import type { ToSMetadata } from "./scrape/tos-metadata.schema.js";
import {
  createObservability,
  createNeptuneConnector,
  RETAILER_CONNECTOR_ENTRIES,
...
function getTosDir(): string {
  return join(__dirname, "..", "tos-metadata");
}

function loadTos(sourceId: string): ToSMetadata | undefined {
  const tosPath = join(getTosDir(), `${sourceId}.json`);
  if (!existsSync(tosPath)) return undefined;
  const raw = readFileSync(tosPath, "utf8");
  return JSON.parse(raw) as ToSMetadata;
}
...
async function main() {
  ...
  const connectors = [
    ...
    ...RETAILER_CONNECTOR_ENTRIES.map(({ sourceId, createConnector }) =>
      createConnector(loadTos(sourceId))
    ),
  ];
  ...
  let results: ConnectorResult[] = await Promise.all(
    connectors.map((connector) => connector.run({ killSwitch: false, observability }))
  );
  ...
  if (usePlaywright) {
    ...
    for (let i = 0; i < connectorIds.length; i++) {
      ...
      if (!PLAYWRIGHT_CONNECTOR_IDS.has(id)) continue;
      try {
  ...
  const bySource: Array<{ source: string; cigars: number; pricing: number; links: number; docs: number; docTypes: { attribute: number; review: number } }> = [];
